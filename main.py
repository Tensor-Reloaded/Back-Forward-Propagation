...
def main():
    ...
    parser.add_argument('--backforward_epochs', default=False, type=int, help='How many epochs to train using the backforward propagation method')
    parser.add_argument('--backforward_lr', default=1.0, type=float, help='What lr to use with the backforward propagation method')
    parser.add_argument('--backforward_train_batch_size', default=256,type=int, help='training batch size')
...
class Solver(object):
    ...
    def perform_backforward(self):
        ...
        backforward_params.epoch = self.args.backforward_epochs
        backforward_params.backforward_epochs = False

        backforward_params.lr = self.args.backforward_lr
        backforward_params.train_batch_size = self.args.backforward_train_batch_size
        backforward_params.save_dir = self.args.save_dir + "_backforward"

        backforward_solver = Solver(backforward_params)
        backforward_solver.init()
        backforward_solver.backforward = True

        modules = list(backforward_solver.model.children())
        i = 0
        while i<len(modules):
        ...
        self.model.load_state_dict(backforward_solver.model.state_dict())
    ...
    def train(self):
        ...
        for batch_num, (data, target) in enumerate(self.train_loader):
            ...
            if self.backforward:
                ...
                X = torch.autograd.Variable(data,requires_grad=True)
                ...
                for i,module in enumerate(self.modules):
                    if i > 0:
                        ...
                        if not hasattr(module, 'weight'):
                            X = y
                            continue
                        print(module.weight.grad.data.size())
                        print(torch.autograd.grad(y,X, module.grad_output,retain_graph=True)[0].size())
                        module.weight.grad.data = torch.autograd.grad(y,X, module.grad_output)[0]
                ...
                #         print(next(module.parameters()).name)
                #         module.weight.grad = torch.autograd.grad(y,X,module.weight.grad)
    ...
    def run(self):
        if self.args.backforward_epochs != False:
            self.perform_backforward()
