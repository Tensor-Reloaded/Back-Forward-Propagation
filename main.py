...
# TODO: Move
def set_to_none(layers: Union[Sequence[nn.modules], nn.modules]) -> None:
    if isinstance(layers, (list, tuple)):
        [set_to_none(x) for x in layers]
    if hasattr(layers, "bias"):
        layers.bias.grad = None
    if hasattr(layers, "weight"):
        layers.weight.grad = None
...
class Solver:
    ...
    def init(self):
        ...
        if hasattr(self.args, "backforward"):
            self.backforward = self.args.backforward
        self.init_dataset()
        self.init_model()
        self.init_optimizer()
        self.init_scheduler()
    ...
    def simple_train(self):
        ...
        for data, target in self.prepare_loader(self.train_loader):
            ...
            if self.backforward:
                ...
                set_to_none(self.model.fc2)
                output = self.model(data)
                loss = self.criterion(output, target)
                loss.backward()
                self.train_maybe_clip_grad()
                set_to_none(self.model.fc1)
    ...
    def train(self):
        # For backforward:
        # I. Grad Scaler: Affects the loss generated by one batch if learnt over and over. If enabled, the loss is more
        # stable.
